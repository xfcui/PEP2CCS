...
class Embedding(nn.Module):
    ...
    def forward(self, seq, charge, data):
        # data: length
        data = data.unsqueeze(-1)
        charge_data = torch.concat([charge, data], dim = 1)
        charge_data = self.mlp(charge_data)
...
class Regression(nn.Module):
    ...
    def forward(self, seq, mz):
        final_state = torch.cat([seq, mz], dim=1)
...
class PEP2CCS(nn.Module):
    ...
    def forward(self, seq, charge, length, mz, ccs2):
        ...
        seq = self.Embedding(seq, charge, length)
        seq = self.Encoder(seq, mask)

        seq = self.Pooling(seq, mask)
        out = self.Regression(seq, torch.sqrt(mz)) + ccs2
        return out
