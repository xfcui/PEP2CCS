...
class Embedding(nn.Module):
    ...
    def forward(self, seq, charge, length, mz):
        length = length.unsqueeze(-1)
        charge_data = torch.concat([charge, length, mz], dim = 1)
...
class PEP2CCS(nn.Module):
    ...
    def forward(self, seq, charge, length, mz, ccs2):
        ...
        seq = self.Embedding(seq, charge, length, mz)
        seq = self.Encoder(seq, mask)

        seq = self.Pooling(seq, mask)
        out = self.Regression(seq) + ccs2
        return out
