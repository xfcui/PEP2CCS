...
def main():
    ...
    try:
        chkp = torch.load(args.model_path)
    ...
    df.to_csv('predictions_with_errors.csv', index=False)
    df.to_csv('./src/Exp2/predictions_with_errors_PEP2CCS_{}.csv'.format(use_full_data), index=False)
